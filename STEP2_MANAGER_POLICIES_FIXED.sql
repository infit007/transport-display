-- FleetSignage Manager Role Setup - Step 2 (CORRECTED)
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM public.profiles WHERE email = 'manager@fleetsignage.com') INTO user_exists;
    ...
    -- Get the user ID
    SELECT id INTO user_id FROM public.profiles WHERE email = 'manager@fleetsignage.com';
    
    -- Assign manager role (ignore if already exists)
    INSERT INTO public.user_roles (user_id, role, created_at)
    VALUES (user_id, 'manager', NOW())
