  ...
  const Display = ({ busNumber, depot }) => {
    ...
    const loadBusData = async () => {
      try {
        ...
        // No fallback - map will not load without valid data
      }
    };
    ...
    useEffect(() => {
      ...
      const onTime = () => {
        try {
          ...
          if (v.currentTime >= v.duration - 0.1 && !hasAdvanced) {
            console.log('⚠ Video near end, advancing as safety measure');
            hasAdvanced = true;
            advancePlaylist();
          }
        } catch {}
      ...
      const scheduleStallAdvance = () => {
        ...
        stallTimer = setTimeout(() => {
          if (!hasAdvanced) {
            console.log('⚠ Video stall timeout, advancing');
            hasAdvanced = true;
            advancePlaylist();
          }
        }, 5000);
    ...
    useEffect(() => {
      ...
      const t = setTimeout(() => {
        ...
        try {
          ...
          if (!canPlay) {
            console.warn('⏱ Video startup watchdog advancing (not playable within 8s)');
            advancePlaylist();
          }
        } catch {
          advancePlaylist();
