...
const MapboxMap = ({ 
  ...
  // 🧭 DEBUG LOGGING
  console.group('🗺️ Mapbox Location Debug');
  ...
  useEffect(() => {
    ...
    const safeCenter = isValidLngLat(currentLocation)
      ? [currentLocation.lng, currentLocation.lat]
      : [78.9568, 29.2138]; // Default India center if missing
  ...
  useEffect(() => {
    if (!mapLoaded || !map.current) return;
    const m = map.current;
    if (!isValidLngLat(endLocation)) {
      console.warn('⚠️ Invalid endLocation:', endLocation);
      return;
    }

    const start = isValidLngLat(currentLocation)
      ? currentLocation
      : startLocation;

    if (!isValidLngLat(start)) {
      console.warn('⚠️ Invalid start/current location:', start);
      return;
    }
    ...
    const drawRoute = async () => {
      try {
        ...
        if (!Array.isArray(coords) || coords.length < 2) {
          console.warn('⚠️ OSRM returned no valid route, drawing straight line');
          m.addSource('route', {
            type: 'geojson',
            data: {
              type: 'Feature',
        ...
        (coords || [[start.lng, start.lat], [endLocation.lng, endLocation.lat]]).forEach((p) => {
          try { bounds.extend(p); } catch {}
        });
  ...
  // 🧍‍♂️ Update current location marker
  useEffect(() => {
    if (!mapLoaded || !map.current) return;
    if (!isValidLngLat(currentLocation)) {
      console.warn('⚠️ Invalid currentLocation for marker:', currentLocation);
      return;
    }
    ...
    console.log('📍 Updated current marker:', currentLocation);
  ...
  useEffect(() => {
    if (!mapLoaded || !map.current) return;
    if (!isValidLngLat(startLocation) || !isValidLngLat(endLocation)) {
      console.warn('⚠️ Invalid start or end location:', { startLocation, endLocation });
      return;
    }

    document.querySelectorAll('.start-marker, .end-marker').forEach(marker => marker.remove());

    const startMarker = new mapboxgl.Marker({ color: '#4CAF50', className: 'start-marker' })
      .setLngLat([startLocation.lng, startLocation.lat])
      .addTo(map.current);

    startMarker.setPopup(
      new mapboxgl.Popup({ offset: 25 }).setHTML(`<div style="color:#000;font-size:12px;"><strong>Start: ${startLocation.name || 'Origin'}</strong></div>`)
    );

    const endMarker = new mapboxgl.Marker({ color: '#F44336', className: 'end-marker' })
      .setLngLat([endLocation.lng, endLocation.lat])
      .addTo(map.current);

    endMarker.setPopup(
      new mapboxgl.Popup({ offset: 25 }).setHTML(`<div style="color:#000;font-size:12px;"><strong>End: ${endLocation.name || 'Destination'}</strong></div>`)
    );

    console.log('✅ Added start and end markers:', { startLocation, endLocation });
  }, [mapLoaded, startLocation, endLocation]);

  return (
    <div className="mapbox-container">
      <div ref={mapContainer} className="mapbox-map" style={{ height: '100%', width: '100%' }} />
      <div className="mapbox-attribution">
        © <a href="https://www.mapbox.com/about/maps/">Mapbox</a> © <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>
      </div>
    </div>
